...
// NOTE: Avoid using ! too much as it can hide real bugs and prefer if-checks, optional chaining, or default values
...
// let btn = document.getElementById('submit'); // CTE: 'btn' is possibly 'null'
let btn = document.getElementById('submit')!; // TypeScript trusts you; no CTE, but runtime error if btn is null or undefined
...
// Example 2
let userType = ['admin', 'viewer'];
let adminUser = userType.find((type) => type === 'admin');
console.log(adminUser!.toUpperCase()); // adminUser! means: I am sure adminUser is not undefined
